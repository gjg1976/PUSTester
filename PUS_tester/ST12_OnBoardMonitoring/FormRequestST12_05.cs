...
{
    public partial class FormRequestST12_05 : Form
    {
        ...
        public List<NumericUpDown> monIntList = new List<NumericUpDown>();
        public List<Label> monIntLabelList = new List<Label>();
        ...
        void updatePayloadAndGUI()
        {
            ...
            for (int j = 0; j < numericUpDownN.Value; j++)
            {
                ...
                UInt16 monInt = decimal.ToUInt16(monIntList[j].Value);
                ...
                payload.Add((byte)(monInt >> 8));
                payload.Add((byte)(monInt & 0xff));
                ...
                offset += 30;
                monIntLabelList[j].Location = new System.Drawing.Point(150, offset + 3);
                monIntList[j].Location = new System.Drawing.Point(250, offset);
                offset += 30;
            ...
            if (numericUpDownN.Value != PMONIDList.Count)
            {
                if (numericUpDownN.Value < PMONIDList.Count)
                {
                    for (int i = PMONIDList.Count - 1; numericUpDownN.Value < PMONIDList.Count; i--)
                    {
                        ...
                        this.Controls.Remove(monIntLabelList[i]);
                        paramIDLabelList.Remove(monIntLabelList[i]);
                ...
                else if (numericUpDownN.Value > PMONIDList.Count)
                {
                    for (int i = PMONIDList.Count; numericUpDownN.Value > PMONIDList.Count; i++)
                    {
                        ...
                        Label tmpLabelMonInt = new Label();
                        tmpLabelMonInt.Text = "Mon interval #" + i.ToString();
                        tmpLabelMonInt.Size = new System.Drawing.Size(100, 20);
                        tmpLabelMonInt.Location = new System.Drawing.Point(60, i * 60 + 212);
                        monIntLabelList.Add(tmpLabelMonInt);
                        this.Controls.Add(tmpLabelMonInt);
                        NumericUpDown tmpMonInt = new NumericUpDown();
                        tmpMonInt.Maximum = new decimal(new int[] {
                            65535,
                            0,
                            0,
                            0});
                        tmpMonInt.Size = new System.Drawing.Size(68, 20);
                        tmpMonInt.Value = 0;
                        tmpMonInt.ValueChanged += new System.EventHandler(this.numericUpDownGeneric_ValueChanged);
                        monIntList.Add(tmpMonInt);
                        this.Controls.Add(tmpMonInt);
