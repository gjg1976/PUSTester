...
{
    public partial class FormRequestST06_09 : Form
    {
        public List<TextBox> addressList = new List<TextBox> ();
        public List<Label> addressLabelList = new List<Label>();
        public List<NumericUpDown> lengthList = new List<NumericUpDown>();
        public List<Label> lengthLabelList = new List<Label>();
        public List<byte> payload = new List<byte>();
        private List<byte> new_payload = null;
        public FormRequestST06_09()
        {
            InitializeComponent();
            ...
            if(new_payload.Count > 0)
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    ...
                    Label tmpLengthLabel = new Label();
                    tmpLengthLabel.Text = "Lenght #" + i.ToString();
                    tmpLengthLabel.Size = new System.Drawing.Size(60, 20);
                    tmpLengthLabel.Location = new System.Drawing.Point(250, i * 30 + 202);
                    lengthLabelList.Add(tmpLengthLabel);
                    this.Controls.Add(tmpLengthLabel);
                    NumericUpDown tmpLength = new NumericUpDown();
                    tmpLength.Maximum = new decimal(new int[] {
                            65535,
                            0,
                            0,
                            0});
                    tmpLength.Value = length;
                    tmpLength.Size = new System.Drawing.Size(68, 20);
                    tmpLength.Location = new System.Drawing.Point(310, i * 30 + 200);
                    tmpLength.ValueChanged += new System.EventHandler(this.numericUpDownGeneric_ValueChanged);
                    lengthList.Add(tmpLength);
                    this.Controls.Add(tmpLength);
                    offset += 10;
                }
            }
            updatePayloadAndGUI();
        ...
        void updatePayloadAndGUI()
        {
            ...
            for (int j = 0; j < numericUpDownN.Value; j++)
            {
                ...
                ushort length = Convert.ToUInt16(lengthList[j].Value);
            ...
            if (numericUpDownN.Value != addressList.Count)
            {
                if (numericUpDownN.Value < addressList.Count)
                {
                    for (int i = addressList.Count - 1; numericUpDownN.Value < addressList.Count; i--)
                    {
                        ...
                        this.Controls.Remove(lengthList[i]);
                        lengthList.Remove(lengthList[i]);

                        this.Controls.Remove(lengthLabelList[i]);
                        lengthLabelList.Remove(lengthLabelList[i]);
                    }
                }
