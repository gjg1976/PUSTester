...
{
    public partial class FormRequestST04_04 : Form
    {
        ...
        public FormRequestST04_04()
        {
            ...
            if(new_payload.Count == 4)
            {
                    ...
                    var value = BitConverter.ToUInt32(payload, 0);
        ...
        {
            payload.Clear();
            byte[] intervalBytes = BitConverter.GetBytes((UInt32)decimal.ToUInt32(numericUpDownInterval.Value));
            if (BitConverter.IsLittleEndian)
                Array.Reverse(intervalBytes);
            payload.Add(intervalBytes[0]);
            payload.Add(intervalBytes[1]);
            payload.Add(intervalBytes[2]);
            payload.Add(intervalBytes[3]);
