...
{
    public partial class FormRequestST23_14 : Form
    {
        ...
        public FormRequestST23_14()
        {
            ...
            {
                Byte[] payloadfileID = { new_payload[offset], new_payload[offset + 1]};
                if (BitConverter.IsLittleEndian)
                    Array.Reverse(payloadfileID);
                UInt32 fileID = BitConverter.ToUInt16(payloadfileID, 0);
                numericUpDownOpsID.Value = fileID;
                offset += 2;
                ...
                Byte[] payloadToPathSize = { new_payload[offset], new_payload[offset + 1] };
                if (BitConverter.IsLittleEndian)
                    Array.Reverse(payloadToPathSize);
                UInt16 filepathToSize = BitConverter.ToUInt16(payloadToPathSize, 0);
                Byte[] filepathTo = new byte[filepathToSize];
                offset += 2;
                for (int j = 0; j < filepathToSize; j++, offset++)
                    filepathTo[j] = new_payload[offset];

                Byte[] payloadfileToSize = { new_payload[offset], new_payload[offset + 1] };
                if (BitConverter.IsLittleEndian)
                    Array.Reverse(payloadfileToSize);
                UInt16 filenameToSize = BitConverter.ToUInt16(payloadfileToSize, 0);
                Byte[] filenameTo = new byte[filenameToSize];
                offset += 2;
                for (int j = 0; j < filenameToSize; j++, offset++)
                    filenameTo[j] = new_payload[offset];

                textBoxTofilePath.Text = System.Text.Encoding.UTF8.GetString(filepathTo) +
                                        System.Text.Encoding.UTF8.GetString(filenameTo);
            }
        }
        ...
        void updatePayload()
        {
            ...
            UInt16 opsID = Convert.ToUInt16(numericUpDownOpsID.Value);

            payload.Add((byte)((opsID >> 8) & 0xff));
            payload.Add((byte)(opsID & 0xff));
            ...
            string tofilename = Path.GetFileName(textBoxTofilePath.Text);
            string tofilepath = "";
            if (tofilename.Length > 0)
                tofilepath = textBoxTofilePath.Text.Replace(tofilename, "");

            count = (UInt16)tofilepath.Length;

            payload.Add((byte)(count >> 8));
            payload.Add((byte)(count & 0xff));

            byte[] bytesToFilepath = Encoding.ASCII.GetBytes(tofilepath);
            foreach (byte b in bytesToFilepath)
                payload.Add(b);

            count = (UInt16)tofilename.Length;

            payload.Add((byte)(count >> 8));
            payload.Add((byte)(count & 0xff));

            byte[] bytesToFilename = Encoding.ASCII.GetBytes(tofilename);
            foreach (byte b in bytesToFilename)
                payload.Add(b);
