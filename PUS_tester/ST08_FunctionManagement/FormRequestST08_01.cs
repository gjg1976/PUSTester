...
{
    public partial class FormRequestST08_01 : Form
    {
        ...
        public FormRequestST08_01()
        {
            InitializeComponent();
            this.new_payload = StaticPayload.PayloadList;
            textBoxFunctionName.MaxLength = Properties.Settings.Default.ST8FunctionSize;
            textBoxFunctionArg.MaxLength = Properties.Settings.Default.ST8FunctionArgSize * 2;

            if (new_payload.Count < Properties.Settings.Default.ST8FunctionSize + Properties.Settings.Default.ST8FunctionArgSize)
                return;

            Byte[] functionName = new byte[Properties.Settings.Default.ST8FunctionSize];
            Byte[] functionArg = new byte[Properties.Settings.Default.ST8FunctionArgSize];
            for(int i = 0; i < Properties.Settings.Default.ST8FunctionSize; i++)
            {
                functionName[i] = new_payload[i];
            }

            textBoxFunctionName.Text = System.Text.Encoding.UTF8.GetString(functionName);
            
            for (int i = 0; i < Properties.Settings.Default.ST8FunctionArgSize; i++)
            {
                functionArg[i] = new_payload[Properties.Settings.Default.ST8FunctionSize + i];

            }
            string logPayload = "";
            for (int i = 0; i < functionArg.Length; i++)
            {
                logPayload += string.Format("{0:x2}", functionArg[i]);
            }
            textBoxFunctionArg.Text = logPayload;
            updatePayload();
        }

        void updatePayload()
        {
            if(Regex.IsMatch(textBoxFunctionArg.Text, @"^[A-Fa-f0-9]*$") == false)
                return;
            payload.Clear();
            ...
            for (; i < Properties.Settings.Default.ST8FunctionSize; i++)
            {
                payload.Add(0);
            }

            i = 0;
            if (textBoxFunctionArg.Text.Length % 2 == 0)
            {
                byte[] values = new byte[textBoxFunctionArg.Text.Length / 2];
                foreach (string s in SplitInParts(textBoxFunctionArg.Text, 2))
                    values[i++] = Convert.ToByte(s, 16);
                for (i = 0; i < values.Length; i++)
                    payload.Add(values[i]);

            }
            for (; i < Properties.Settings.Default.ST8FunctionArgSize; i++)
            {
                payload.Add(0);
            }
