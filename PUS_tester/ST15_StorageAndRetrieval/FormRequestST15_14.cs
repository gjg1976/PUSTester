...
{
    public partial class FormRequestST15_14 : Form
    {
        ...
        public FormRequestST15_14()
        {
            ...
            {
                int offset = 0;
                Byte[] payloadTime = { new_payload[offset], new_payload[offset + 1], new_payload[offset + 2], new_payload[offset + 3] };
                if (BitConverter.IsLittleEndian)
                    Array.Reverse(payloadTime);
                UInt32 time = BitConverter.ToUInt32(payloadTime, 0);
                offset +=4;
                var currentValueFieldTime = numericUpDown2.GetType().GetField("currentValue", BindingFlags.Instance | BindingFlags.NonPublic);
                if (currentValueFieldTime != null)
                {
                    currentValueFieldTime.SetValue(numericUpDown2, Convert.ToDecimal(time));
                    numericUpDown2.Text = time.ToString();
                }
                ...
                for (int i = 0; i < N1; i++)
                 {
                    ...
                    tmpLabel.Location = new System.Drawing.Point(100, i * 30 + 212);
                    ...
                    tmp.Location = new System.Drawing.Point(190, i * 30 + 210);
        ...
        {
            payload.Clear();
            UInt32 time = decimal.ToUInt32(numericUpDown2.Value);
            payload.Add((byte)(time >> 24));
            payload.Add((byte)(time >> 16));
            payload.Add((byte)(time >> 8));
            payload.Add((byte)(time & 0xff));
