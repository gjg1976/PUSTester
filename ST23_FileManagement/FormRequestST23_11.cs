...
{
    public partial class FormRequestST23_11 : Form
    {
        ...
        public FormRequestST23_11()
        {
            ...
            if (new_payload.Count > 0)
            {
                ...
                Byte[] payloadDireNewSize = { new_payload[offset], new_payload[offset + 1] };
                if (BitConverter.IsLittleEndian)
                    Array.Reverse(payloadDireNewSize);
                UInt16 dirNewSize = BitConverter.ToUInt16(payloadDireNewSize, 0);
                Byte[] dirnewname = new byte[dirNewSize];
                offset += 2;
                for (int j = 0; j < dirNewSize; j++, offset++)
                    dirnewname[j] = new_payload[offset];

                textBoxDirPath.Text = System.Text.Encoding.UTF8.GetString(filepath);
                textBoxDirOldName.Text = System.Text.Encoding.UTF8.GetString(filename);
                textBoxDirNewName.Text = System.Text.Encoding.UTF8.GetString(dirnewname);
            }
        }
        ...
        {
            payload.Clear();
            string searchPath = textBoxDirPath.Text;
            string searchString = textBoxDirOldName.Text;
            string dirNewNameString = textBoxDirNewName.Text;

            UInt16 count = (UInt16)searchPath.Length;

            payload.Add((byte)(count >> 8));
            payload.Add((byte)(count & 0xff));

            byte[] bytesFilepath = Encoding.ASCII.GetBytes(searchPath);
            foreach (byte b in bytesFilepath)
                payload.Add(b);

            count = (UInt16)searchString.Length;

            payload.Add((byte)(count >> 8));
            payload.Add((byte)(count & 0xff));

            byte[] bytesFilename = Encoding.ASCII.GetBytes(searchString);
            foreach (byte b in bytesFilename)
                payload.Add(b);

            count = (UInt16)dirNewNameString.Length;

            payload.Add((byte)(count >> 8));
            payload.Add((byte)(count & 0xff));

            byte[] bytesDirNewName = Encoding.ASCII.GetBytes(dirNewNameString);
            foreach (byte b in bytesDirNewName)
                payload.Add(b);
        ...
        private void textBoxDirNewName_TextChanged(object sender, EventArgs e)
        {
            updatePayload();
        }
    }
